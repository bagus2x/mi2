...
import { HiArrowDownLeft, HiChevronDown } from 'react-icons/hi2'
  ...
  return (
    <div className='flex items-center justify-center md:invisible'>
      <button className='h-10 w-10 rounded-full' onClick={() => setOpen(true)}>
        <HiMenu size={24} className='m-2 text-gray-800' />
      </button>
      <div
        className={clsx(
...
function MobileMenuItem({ text, icon: Icon, subItems }: NavbarItem) {
  const [open, setOpen] = useState(false)
  const pathname = usePathname()
  ...
  return (
    <li className='flex w-full flex-col space-y-2'>
      <button
        className={clsx(
          'flex w-full items-center space-x-2 rounded-xl px-4 py-2 transition-all hover:bg-green-500 hover:text-white',
        )}
                  ...
                  <Menu.Items className='absolute right-0 mt-4 flex flex-col rounded-lg bg-white p-2 shadow-lg outline outline-1 outline-gray-100 transition'>
                    {subItems.map((subItem, index) => (
                      <Link
                        ...
                        href={subItem.href}
                        onClick={() => close()}
                        className='flex select-none items-center gap-2 rounded-xl px-4 py-2 text-sm text-gray-900 transition hover:bg-green-500 hover:text-white'
                      >
                        {subItem.text}
                      </Link>
                    ))}
