...
export default function PostList({ initialData }: PostListProps) {
  const { isLoading, isFetching, data, fetchNextPage } = useGetUsers(initialData)
  ...
  return (
    <section className="w-full min-h-full flex flex-col relative">
      <ul className="w-full flex flex-col space-y-8">
        <AnimatePresence>
          {data?.pages.map((page) => (
            page.data.map((post, index) => (
              <PostItem key={post.id} post={post} animationDelay={index * 0.1} />
            ))
          ))}
        </AnimatePresence>
